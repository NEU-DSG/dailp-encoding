...
type ContentPageQuery = { readonly wpPage: Maybe<Pick<WpPage, 'title' | 'content'>> };
...
type homesneadpiedailpEncodingwebsitesrcmenuTsx1475177QueryVariables = Exact<{ [key: string]: never; }>;


type homesneadpiedailpEncodingwebsitesrcmenuTsx1475177Query = { readonly wpMenu: Maybe<{ readonly menuItems: Maybe<{ readonly nodes: Maybe<ReadonlyArray<Maybe<(
        Pick<WpMenuItem, 'path' | 'label'>
        & { readonly childItems: Maybe<{ readonly nodes: Maybe<ReadonlyArray<Maybe<Pick<WpMenuItem, 'path' | 'label'>>>> }> }
      )>>> }> }> };
