...
export function useDailpAuthOperations(
  ...
  const [refreshTokenMutationResult, refreshTokenMutation] =
    Dailp.useRefreshTokenMutation()
  const [logoutMutationResult, logoutMutation] = Dailp.useLogoutMutation()
  const [requestPasswordResetMutationResult, requestPasswordResetMutation] =
    Dailp.useRequestPasswordResetMutation()
  const [resetPasswordMutationResult, resetPasswordMutation] =
  ...
  async function resetPassword(email: string) {
    ...
    alert(
      response.data?.requestPasswordReset.message || "Password reset code sent!"
    )
  }
  ...
  async function changePassword(token: string, newPassword: string) {
    ...
    alert(
      response.data?.resetPassword.message || "Password reset successfully!"
    )
  }
  ...
  async function confirmUser(email: string, confirmationCode: string) {
    try {
      ...
      // Success
      alert(
        response.data?.verifyEmail.message || "Email verified successfully!"
      )
      await navigate("/auth/login")
    } catch (err) {
  ...
  async function setupTokenRefresh(): Promise<number> {
    ...
    try {
      ...
      if (intervalLength <= 0) {
        ...
        const newIntervalLength =
          newDecoded.exp * 1000 - Date.now() - 5 * 60 * 1000
...
export function roleToGroups(role: string): UserGroup[] {
  ...
  switch (role) {
    case "Contributors":
      return [UserGroup.Contributors]
    case "Editors":
      return [UserGroup.Editors]
    case "Readers":
      return [UserGroup.Readers]
    default:
      return [UserGroup.Readers]
  }
}
