...
select coalesce(inserted_gloss.id, global_gloss.id), word_id, index, morpheme, segment_type from unnest($1::uuid[], $2::text[], $3::uuid[], $4::bigint[], $5::text[], $6::segment_type[]) as input_data(doc_id, gloss, word_id, index, morpheme, segment_type)
