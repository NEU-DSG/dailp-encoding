...
inserted_gloss as (
  insert into morpheme_gloss (document_id, gloss)
    select document_id, gloss from input_data
...
insert into word_segment (gloss_id, word_id, index_in_word, morpheme, segment_type)
-- Fill in glosses that weren't inserted with their global match.
select coalesce(inserted_gloss.id, global_gloss.id), word_id, index, morpheme, segment_type
from input_data
  left join inserted_gloss on inserted_gloss.document_id = input_data.document_id and inserted_gloss.gloss = input_data.gloss
