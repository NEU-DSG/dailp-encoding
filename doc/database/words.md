...
| `currently_editing`                  | `boolean`                | True while a user holds the editing lock on this word. Defaults to `false`.                      |
...
| `editing_user_id`                    | `uuid? -> dailp_user`    | User holding the editing lock. Set to `null` when no lock is held.                                  |
| `editing_lock_token`                 | `uuid?`                  | Per-session token (generated client-side) proving ownership of the lock for save and release.       |

- One of `page_id` or `character_range` must be supplied
- The `currently_editing` / `editing_started_at` / `editing_user_id` / `editing_lock_token` columns implement per-word editing locks. See `types/queries/acquire_word_lock.sql`, `release_word_lock.sql`, and the `editing_lock_token` check in `update_word.sql`.
