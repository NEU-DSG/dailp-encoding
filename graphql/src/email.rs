...
fn get_base_url() -> String {
    env::var("DAILP_API_URL").unwrap_or_else(|_| "http://localhost:3001".to_string())
}

/// Send email verification code to user
pub async fn send_verification_email(to_email: &str, token: &str) -> Result<()> {
    let from_email = env::var("SMTP_FROM").context("SMTP_FROM not set in environment")?;
    let from_name = env::var("SMTP_FROM_NAME").unwrap_or_else(|_| "DAILP".to_string());
...
async fn send_email(
    ...
    let smtp_username =
        env::var("SMTP_USERNAME").context("SMTP_USERNAME not set in environment")?;
    let smtp_password =
        env::var("SMTP_PASSWORD").context("SMTP_PASSWORD not set in environment")?;
