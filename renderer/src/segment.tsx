      ...
      <AnnotatedWord id={`w${segment.index}`}>
        ...
        {level > ExperienceLevel.Beginner ? (
          <MorphemicSegmentation
            segments={segment.segmentation}
            dialog={dialog}
            onOpenDetails={onOpenDetails}
          />
        ) : null}
        <div>{segment.englishGloss.join(", ")}</div>
      </AnnotatedWord>
    )
  } else if (ty.endsWith("AnnotatedPhrase")) {
    const children = segment.parts?.map((seg, i) => (
      <Segment
        key={i}
        segment={seg}
        dialog={dialog}
    ...
    if (segment.ty === "BLOCK") {
      ...
      return (
        <div>
          ...
          <TranslationPara>
            {translationBlock?.segments.join(". ") || null}.
          </TranslationPara>
        </div>
      )
    } else {
 ...
 */
const MorphemicSegmentation = ({ segments, dialog, onOpenDetails }) => {
...
const MorphemeSegment = ({ segment, dialog, onOpenDetails }) => (
  <MorphemeButton {...dialog} onClick={() => onOpenDetails(segment)}>
    {segment.morpheme}
  </MorphemeButton>
)
