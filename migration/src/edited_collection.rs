...
pub async fn migrate_edited_collection(db: &Database) -> anyhow::Result<()> {
    let res = SheetResult::from_sheet("12R07Ks8A5g2jffqoILJw7nRn6GehwNwdwosxaJBwNNM", None)
        .await?
        .into_collection_index(&"Cherokees Writing the Keetoowah Way".to_string(), &4579, &"cwkw".to_string())?;
    let collection = res;
    ...
    let chapter_result: Vec<Chapter> = chapter_group_one.into_iter().chain(chapter_group_two).collect();

    db.insert_all_chapters(chapter_result, collection.slug).await?;
