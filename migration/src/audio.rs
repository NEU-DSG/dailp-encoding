    ...
    pub async fn new(client: &Client, drs_id: &str) -> Result<Self, anyhow::Error> {
        ...
        Ok(loop {
            ...
            if let Err(e) = &response_result {
                ...
                println!("Network error fetching DRS ID '{}': {}. Retrying (attempt {}/8)", 
                    drs_id, e, tries + 1);
            ...
            if !status.is_success() {
                ...
                println!("DRS API returned error {} for ID '{}'. Retrying (attempt {}/8)", 
                    status, drs_id, tries + 1);
                sleep(Duration::from_millis(1000 * 2_u64.pow(tries))).await;
                tries += 1;
                continue;
            }
            ...
            if tries > 7 {
                break Err(anyhow::anyhow!(
                    ...
                    drs_id, json_result.unwrap_err()
                ));
            }
            
            println!("JSON parsing error for DRS ID '{}'. Retrying (attempt {}/8)", 
                drs_id, tries + 1);
            sleep(Duration::from_millis(1000 * 2_u64.pow(tries))).await;
            tries += 1;
        }?)
    ...
    pub async fn new(
        ...
        let audio_url = if is_drs_key(audio_ref_key) {
            let drs_content = DrsRes::new(&client, audio_ref_key).await
                .map_err(|e| anyhow::anyhow!("Failed to fetch DRS content for audio key '{}': {}", audio_ref_key, e))?;
            let url = drs_content.get_url();
            if url.is_empty() {
                return Err(anyhow::anyhow!("DRS resource '{}' returned empty URL", audio_ref_key));
            }
            url
        } else {
        ...
        let annotations = match annotation_ref_key {
            ...
            Some(key) => {
                ...
                let annotation_url = if is_drs_key(key) {
                    let drs_content = DrsRes::new(&client, key).await
                        .map_err(|e| anyhow::anyhow!("Failed to fetch DRS content for annotation key '{}': {}", key, e))?;
                    let url = drs_content.get_url();
                    if url.is_empty() {
                        return Err(anyhow::anyhow!("DRS resource '{}' returned empty URL for annotations", key));
                    }
                    url
                } else {
                ...
                match result {
                    ...
                    std::result::Result::Err(e) => {
                        println!("Warning: Failed to fetch annotations from '{}': {}", annotation_url, e);
                        None
                    }
                }
            }
        ...
        for (line_index, record_result) in reader.deserialize::<AudioAnnotationRow>().enumerate() {
            ...
            if annotation.start_time < 0.0 || annotation.end_time < 0.0 {
                return Err(anyhow::anyhow!(
                    ...
                    line_index + 1, annotation.start_time, annotation.end_time
                ));
            }
            ...
            if annotation.start_time >= annotation.end_time {
                return Err(anyhow::anyhow!(
                    ...
                    annotation.start_time, annotation.end_time, line_index + 1
                ));
            }
